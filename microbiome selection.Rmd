...
mod <- lme(time ~ round * diet * selection , random=~1|line/replicate, method="REML", data = flydata )
summary(mod)
hist(resid(mod))
...
```{r adonis}
d = UniFrac(tax_glom(microbedat, taxrank = "Family"))
...
```{r}
microbedat.hsd <- tax_glom(subset_samples(microbedat, diet == "hsd"), taxrank = "Family")  
...
```{r}
microbedat.nsd <- tax_glom(subset_samples(microbedat, diet == "nsd"), taxrank = "Family")  
...
```{r phyloseq_round}
microbedat.phyloseq.round <- phyloseq_to_deseq2(tax_glom(subset_samples(microbedat, diet == "hsd"), taxrank = "Family"), ~ round + line)
...
microbedat.phyloseq.round <- phyloseq_to_deseq2(tax_glom(subset_samples(microbedat, diet == "nsd"), taxrank = "Family"), ~ round + line)
microbedat.phyloseq.round <- results(DESeq(microbedat.phyloseq.round, test="LRT", fitType="local", reduced = ~  line))
head(microbedat.phyloseq.round[order(microbedat.phyloseq.round$pvalue),])

tax_table(microbedat)[rownames(microbedat.phyloseq.round[which(microbedat.phyloseq.round$padj < 0.05),]),"Family"]
```
