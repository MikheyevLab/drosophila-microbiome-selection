...
effect(term="round:diet:selection", xlevels=list(selection=c("selection","noselection"),diet=c("control","nsd")), mod=mod) %>%
  ...
  scale_color_manual( labels = c("Control", "Selection"), values = c("dodgerblue", "orange")) + ylab("Eclosion time (hours)") + xlab("Round") + guides(alpha = F, color=guide_legend(override.aes=list(fill=NA)), legend.title=element_blank())
